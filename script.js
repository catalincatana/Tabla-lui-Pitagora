function createTable()
{
		var tbody = '';
        var theader = '<div style="position:relative; left:50%; top:30%" id="tabelanimatie" ><table border="1" id="table" >\n';
          //  for($u=1; $u<=2; $u++){
              tbody += '<tr id="linia1">';
					tbody += '<td id="coloana1">';
					tbody += 'x';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '1';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '2';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '3';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '4';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '5';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '6';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '7';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '8';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '9';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '10';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia2">';
					tbody += '<td id="coloana1">';
					tbody += '1';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '1';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '2';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '3';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '4';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '5';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '6';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '7';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '8';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '9';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '10';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia3">';
					tbody += '<td id="coloana1">';
					tbody += '2';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '2';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '4';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '6';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '8';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '10';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '12';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '14';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '16';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '18';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '20';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia4">';
					tbody += '<td id="coloana1">';
					tbody += '3';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '3';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '6';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '9';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '12';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '15';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '18';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '21';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '24';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '27';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '30';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia5">';
					tbody += '<td id="coloana1">';
					tbody += '4';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '4';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '8';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '12';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '16';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '20';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '24';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '28';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '32';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '36';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '40';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia6">';
					tbody += '<td id="coloana1">';
					tbody += '5';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '5';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '10';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '15';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '20';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '25';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '30';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '35';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '40';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '45';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '50';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia7">';
					tbody += '<td id="coloana1">';
					tbody += '6';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '6';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '12';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '18';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '24';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '30';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '36';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '42';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '48';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '54';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '60';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia8">';
					tbody += '<td id="coloana1">';
					tbody += '7';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '7';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '14';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '21';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '28';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '35';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '42';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '49';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '56';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '63';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '70';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia9">';
					tbody += '<td id="coloana1">';
					tbody += '8';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '8';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '16';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '24';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '32';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '40';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '48';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '56';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '64';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '72';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '80';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia10">';
					tbody += '<td id="coloana1">';
					tbody += '9';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '9';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '18';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '27';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '36';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '45';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '54';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '63';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '72';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '81';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '90';
					tbody += '</td>'
			tbody += '</tr>\n';
			tbody += '<tr id="linia11">';
					tbody += '<td id="coloana1">';
					tbody += '10';
					tbody += '</td >'
					tbody += '<td id = "coloana2">';
					tbody += '10';
					tbody += '</td>'
					tbody += '<td id = "coloana3">';
					tbody += '20';
					tbody += '</td>'
					tbody += '<td id = "coloana4">';
					tbody += '30';
					tbody += '</td>'
					tbody += '<td id = "coloana5">';
					tbody += '40';
					tbody += '</td>'
					tbody += '<td id = "coloana6">';
					tbody += '50';
					tbody += '</td>'
					tbody += '<td id = "coloana7">';
					tbody += '60';
					tbody += '</td>'
					tbody += '<td id = "coloana8">';
					tbody += '70';
					tbody += '</td>'
					tbody += '<td id = "coloana9">';
					tbody += '80';
					tbody += '</td>'
					tbody += '<td id = "coloana10">';
					tbody += '90';
					tbody += '</td>'
					tbody += '<td id = "coloana11">';
					tbody += '100';
					tbody += '</td>'
			tbody += '</tr>\n';
    var tfooter = '</table></div>';
    document.getElementById('tabelpitagora').innerHTML = theader + tbody + tfooter;
					//				}
}
